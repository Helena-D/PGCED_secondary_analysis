---
title: 'BEBROAD Plotting external rg: Single PGCED trait'
...
BEBROAD_rgs_non_MVP <- read_csv("/Users/helenadavies/werk/PGC_ED_Freeze3/PGC_ED_F3_github_scripts/secondary_analysis_rg_results/BEBROAD_PGCED3_rgs.csv")
```
# Keep correct traits in rg
```{r keep correct traits}
BEBROAD_rgs_non_MVP <- BEBROAD_rgs_non_MVP %>% 
  filter(p2 == "ADHD06" |
           p2 == "SMOK10" |
           p2 == "ALCD03" |
           p2 == "ALCU01" |
...
BEBROAD_rgs_MVP <- read_csv("/Users/helenadavies/werk/PGC_ED_Freeze3/PGC_ED_F3_github_scripts/secondary_analysis_rg_results/BEBROAD_PGCED3_MVP_rgs_MVP.csv")
```

```{r bind MVP and non MVP}
BEBROAD_rgs <- BEBROAD_rgs_MVP %>%
  bind_rows(BEBROAD_rgs_non_MVP)
```
...
BEBROAD_rgs_results_details <- left_join(BEBROAD_rgs,
                                 trait_details,
                                 by = "p2")

BEBROAD_rgs_results_details

## for now ##
BEBROAD_rgs_results_details <- BEBROAD_rgs_results_details %>%
  mutate(name =
...
```{r no. traits}
n_traits <- nrow(BEBROAD_rgs_results_details)
```
...
BEBROAD_rgs_results_details_ordered <- BEBROAD_rgs_results_details[order(BEBROAD_rgs_results_details$Category), ]

write_excel_csv(BEBROAD_rgs_results_details_ordered,
                file = "/Users/helenadavies/werk/PGC_ED_Freeze3/PGC_ED_F3_github_scripts/secondary_analysis_rg_results/single_trait_rg/BEBROAD_PGCED3_rgs_SUPPLEMENTARY2.csv")
``` 

# Plot: order by strength of rg
Filter for only significant rg for plotting
```{r filter sig rg only plotting}
BEBROAD_rgs_results_details_sig <- BEBROAD_rgs_results_details %>%
  filter(`Bonferroni significant` == "significant")
```

Create column to specify if negative or positive for plotting
```{r direction of rg}
BEBROAD_rgs_results_details_sig <- BEBROAD_rgs_results_details_sig %>%
  mutate(direction =
           case_when(rg > 0 ~ "positive",
                     rg < 0 ~ "negative"))

BEBROAD_rgs_results_details_sig %>%
  count(direction)
```
...
plot <- ggplot(BEBROAD_rgs_results_details_sig,
       aes(x=Trait,
           y=rg,
          color=direction
           )) + 
...
Save rg plot
```{r Save rg plot}
ggsave(filename = "/Users/helenadavies/werk/PGC_ED_Freeze3/PGC_ED_F3_github_scripts/secondary_analysis_rg_results/single_trait_rg/BEBROAD_PGCED3_rgs_plot2.png",
       plot = plot,
       height = 65,
       width = 50,
       units = "cm",
